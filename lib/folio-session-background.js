...
(function () {
  ...
  // Strip common FOLIO API URL prefixes and suffixes to get the "instance" hostname.
  // e.g. "api-michstate-lm.folio.ebsco.com" → "michstate-lm.folio.ebsco.com"
  //      "okapi-other.folio.ebsco.com"      → "other.folio.ebsco.com"
  //      "folio-sunflower-okapi.dev.folio.org" → "folio-sunflower.dev.folio.org"
  function normalizeHost(hostname) {
    var parts = hostname.split(".");
    var first = parts[0];
    // Strip prefixes
    first = first.replace(/^(api-|okapi-|kong-|api\.|okapi\.)/, "");
    // Strip suffixes
    first = first.replace(/(-api|-okapi|-kong)$/, "");
    parts[0] = first;
    return parts.join(".");
  }

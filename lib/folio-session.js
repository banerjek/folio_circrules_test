...
var FolioSession = (function () {
  ...
  function ensureHostPermission(origin) {
    ...
    return new Promise(function (resolve) {
      chrome.permissions.request({ origins: [pattern] }, function (granted) {
        // Check runtime.lastError to suppress "must be called during a user gesture" warnings
        if (chrome.runtime.lastError) {
          console.warn(_logPrefix, "permissions.request:", chrome.runtime.lastError.message);
          resolve(false);
          return;
        }
   ...
   */
  async function _extractFolioSession() {
    var result = { url: null, tenant: null, token: null, debug: [] };

    // 1. IndexedDB (localforage): okapiSess
    try {
      var idbSess = await new Promise(function (resolve, reject) {
      ...
      if (idbSess && typeof idbSess === "object") {
        result.url = idbSess.url || idbSess.okapiUrl || null;
        result.tenant = idbSess.tenant || null;
        result.token = idbSess.token || null;
        result.debug.push("Found okapiSess in IndexedDB (localforage) — tenant: " + (idbSess.tenant || "?"));
      } else if (idbSess != null) {
        result.debug.push("okapiSess in IndexedDB is not an object: " + String(idbSess).substring(0, 100));
      }
    } catch (e) {
      result.debug.push("IndexedDB okapiSess read error: " + e.message);
    }

    // 1b. Fallback: localStorage okapiSess (older FOLIO versions)
    if (!result.tenant) {
      try {
        var stored = localStorage.getItem("okapiSess");
        if (stored && stored !== "true") {
          var sess = JSON.parse(stored);
          result.url = result.url || sess.url || sess.okapiUrl || null;
          result.tenant = sess.tenant || null;
          result.token = result.token || sess.token || null;
          result.debug.push("Found okapiSess in localStorage (JSON)");
        } else if (stored) {
          result.debug.push("okapiSess in localStorage is '" + stored + "' (not a JSON object)");
        }
      } catch (e) {
        result.debug.push("okapiSess localStorage parse error: " + e.message);
      }
    }

    // 1c. localStorage: direct 'tenant' key
    try {
